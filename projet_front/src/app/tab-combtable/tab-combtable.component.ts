...
export class TabCombtableComponent implements OnInit {
  ...
  calculerMargeMensuelle(): void {
    this.transactionService.getMergeForMonth().subscribe((data) => {
  ...
  createChartAnnuelComptable(): void {
    ...
    this.transactionService.getMergeForYear().subscribe(
      (data: any[]) => {
        //console.log('dataYear', data);

        this.marginData = data.map((item) => item.marge);
        console.log('this.marginData', this.marginData);
        this.calculateTotalMarge();
        this.checkNegativeTrimesters();
      ...
      (error) => {
        console.error(
          'Erreur lors de la récupération des données de marge',
          error
        );
      }
    );
  ...
  calculateTotalMarge(): void {
    ...
    if (this.marginData && this.marginData.length > 0) {
      ...
      console.log('marginData',this.marginData);
      
      console.log('total', this.totalMarge);
      ...
      console.error('Aucune donnée de marge disponible');
    }
  }
  ...
  checkNegativeTrimesters(): void {
    ...
    this.trimestersAlerts = trimesters
      .filter((trimestre) => {
